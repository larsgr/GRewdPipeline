...
for(spc in spcs){
  ...
  is111 <- rownames(x) %in% unlist(grps[grps111,spc])
  
  lines(hist(x[is111,],plot=FALSE,breaks=seq(0,20,by=1)),col="red")

  legend("topright",legend=c("In orthogroup","In all species","Single in all"),
         fill=c("lightblue","purple","red"))
...
for(spc in spcs){
  ...
  legend("topright",legend=colnames(exprCnt[[spc]]),
