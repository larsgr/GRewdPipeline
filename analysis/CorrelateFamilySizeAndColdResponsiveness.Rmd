...
We claim in the paper that genes important to cold acclimation tend to have more complex duplication history. The basis for this was that a large part of the Greenup genes (i.e. known cold responsive genes in barley) were not in the HCOGs because of complex gene tree topology (as inferred by manually inspecting each tree). The aim of this analysis is to statistically test if this is a general trend for all cold-response genes by correlating family size with cold response.
...
Part of the pipeline includes splitting large trees and apply "minimum" topology filtering the trees so that: 1) all the pooids form a single clan, and 2) the orthogroup includes at least one basal (StLa, NaSt, MeNu) and 1 "core" (HoVu, BrDi) denovo ortholog.

The following plots show how the filtering correlates cold-response. The second plot only includes the genes with typical mean gene expression.
```{r plotPropFiltered}
DEtbl %>% 
  ...
  ylab("removed by minimum filter") + xlab("cold response")
...
DEtbl %>% 
  ...
  facet_grid( . ~ responseType) +
  scale_y_continuous(labels=percent) +
  ylab("removed by minimum filter") + xlab("cold response") + ggtitle("5 < log2(1+baseMean)  < 10")
```
...
In phylostratigraphic analysis each gene is assigned to a phylostrata that indicates the age of the gene family. The phylostrata is defined by the earliest common ancestor that conatins an ortholog of the given gene. This then used to look for enrichment of certain gene classes (i.e. cold responsive) at the different phylostrata. 

A common method to identify the phylostrata is to blastp against all possible sequences available, and assign the phylostrata based on the hit with the oldest MRCA. However, in this analysis I use the gene-trees downloaded from ensembl plants compara. The age of the oldest internal node in the gene-tree is used as the phylostrata for that tree. The ortholog groups is assigned to a gene-tree (and hence phylostrata) by mapping the matching Brachypodium geneIDs.
...
#   summarise( taxonOrder = max(taxonOrder)) %>%
#   mutate( taxonName = factor(orderedTaxonsTbl$taxonName[taxonOrder], 
#                              levels=orderedTaxonsTbl$taxonName))
...
Here is the number of trees per phylostrata (oldest node in the tree) for all trees that contain a brachypodium gene:
```{r plotTaxonNumbers}
...
Here is the number brachypodium genes (only DE tested genes) per phylostrata:
...
To test for enrichment of cold-responsive genes at the different phylostrata we use odds ratio (OR) as statistic. Odds ratio for phylostrata $p$ and differential expression category $d$ (dirDEG: "down", "up" or "not DE") is calculated as:
...
An odds ratio greater than 1 (log OR > 0) means that there is a higher chance of observing the given DE category in the given phylostrata.

```{r calcOddsRatioPS,cache=T}
  
calcOR <- function(x){
  x %>% 
    add_count(responseType,spc) %>% dplyr::rename(N=n) %>% 
    add_count(responseType,spc,dirDEG) %>% dplyr::rename(Nd=n) %>% 
    add_count(responseType,spc,taxonOrder) %>% dplyr::rename(Np=n) %>% 
    group_by(responseType,spc,dirDEG,taxonOrder) %>% 
    # where: Npd = n(), NpD = Np-Npd, NPd = Nd-Npd, NPD=N-Np-Nd+Npd
    summarise( Npd = n(), NpD = Np[1]-n(), NPd = Nd[1]-n(), NPD=N[1]-Np[1]-Nd[1]+n()) %>% 
    mutate( OR=(Npd/NpD)/(NPd/NPD) )
}

# calc log odds
ORtbl <- calcOR(DEtblPS)


# bootstrap log odds
ORtblBootStrap1000 <-
  replicate(n = 1000,calcOR(DEtblPS %>% sample_n(size=nrow(DEtblPS),replace = T)),simplify = F) %>% 
  bind_rows() 
```


```{r calcLogOddsPS,cache=T}
calcLL <- function(x){
  x %>% 
    add_count(responseType,spc) %>% dplyr::rename(nAny=n) %>% 
    add_count(responseType,spc,dirDEG) %>% dplyr::rename(nDirDEG=n) %>% 
    group_by(responseType,spc,taxonOrder) %>% 
    mutate( pTaxon=n()/nAny ) %>% 
    group_by(dirDEG,responseType,spc,taxonOrder,pTaxon) %>% 
    # estimated probability
    summarise( p=n()/nDirDEG[1] ) %>% 
    mutate(ll=log2(p/pTaxon))
}

# calc log odds
llTbl <- calcLL(DEtblPS)

# bootstrap log odds
llTblBootStrap1000 <-
  replicate(n = 1000,calcLL(DEtblPS %>% sample_n(size=nrow(x),replace = T)),simplify = F) %>% 
  bind_rows() 
```

The following plot shows the log2 odds ratio, i.e. log2( proportion of DEGs in strata / proportion of tested genes of). The error-bars indicate 95% confince interval based on 1000 bootstraps.

```{r plotPhylostrataLogOdds, fig.height=12}

q95lower <- function(x){quantile(x,probs=0.025)}
q95upper <- function(x){quantile(x,probs=0.975)}

llTblBootStrap1000 %>% 
  ggplot(aes(x=taxonOrder,y=ll,color=dirDEG)) +
  stat_summary(geom="errorbar", fun.ymin=q95lower, fun.ymax=q95upper, width=0.1) +
  # stat_summary(geom="point", fun.y=mean) + 
  geom_point(data=llTbl)+geom_line(data=llTbl)+
  ...
  ylab("log2(odds)") + xlab("phylostrata")

```

Here is the same plot but combining all species:

```{r calcLogOddsPSall,cache=T}
  
calcLLall <- function(x){
  x %>% 
    add_count(responseType) %>% dplyr::rename(nAny=n) %>% 
    add_count(responseType,dirDEG) %>% dplyr::rename(nDirDEG=n) %>% 
    group_by(responseType,taxonOrder) %>% 
    mutate( pTaxon=n()/nAny ) %>% 
    group_by(dirDEG,responseType,taxonOrder,pTaxon) %>% 
    # estimated probability
    summarise( p=n()/nDirDEG[1] ) %>% 
    mutate(ll=log2(p/pTaxon))
}
...
# calc log odds
llTblAll <- calcLLall(DEtblPS)

# bootstrap log odds
llTblBootStrap1000all <-
  replicate(n = 1000,calcLLall(DEtblPS %>% sample_n(size=nrow(x),replace = T)),simplify = F) %>% 
  bind_rows() 
```
...
calcORall <- function(x){
  x %>% 
    add_count(responseType) %>% dplyr::rename(N=n) %>% 
    add_count(responseType,dirDEG) %>% dplyr::rename(Nd=n) %>% 
    add_count(responseType,taxonOrder) %>% dplyr::rename(Np=n) %>% 
    group_by(responseType,dirDEG,taxonOrder) %>% 
    # where: Npd = n(), NpD = Np-Npd, NPd = Nd-Npd, NPD=N-Np-Nd+Npd
    summarise( Npd = n(), NpD = Np[1]-n(), NPd = Nd[1]-n(), NPD=N[1]-Np[1]-Nd[1]+n()) %>% 
    mutate( OR=(Npd/NpD)/(NPd/NPD) )
}
...
# calc log odds
ORtblAll <- calcORall(DEtblPS)


# bootstrap log odds
ORtblBootStrap1000all <-
  replicate(n = 1000,calcORall(DEtblPS %>% sample_n(size=nrow(DEtblPS),replace = T)),simplify = F) %>% 
  bind_rows() 
...
Interpretation: The Pooideae and BOP clade phylostrata has too much noise to make and conclusion, but for the rest it seems that cold-responsive genes tend to be underespresented among the most ancient genes. Interrestingly, up-regulated genes seems to be a bit older than the down-regulated genes
