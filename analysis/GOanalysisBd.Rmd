...
par(mar=c(8, 4, 4, 2) + 0.1)
barplot(sigGOnormMat,col=GO2Col[sortedSigGOs],width=0.5, space=c(1,rep(c(1,1,2),3)),
        las=2, xlim=c(0,40), ylab="Proportion of slim GO annotations",
        names.arg=labs)
par(mar=c(5, 4, 4, 2) + 0.1)
legend("topright",legend = rev(getGOterms(sortedSigGOs)),
       fill=rev(GO2Col[sortedSigGOs]))
...
y <- apply(sigGOnormMat,2,function(x){cumsum(x)-x/2})[,-ncol(sigGOnormMat)]
pval <- sigGOpvalmat[rownames(sigGOnormMat),colIdx[-ncol(sigGOnormMat)]]
sigLabel <- cut(pval,c(0,0.005,0.01,0.05,1),labels=c("***","**","*",""))

text(x = 0.2+rep(cumsum(c(1,rep(c(1,1,1.5),3)))[-ncol(sigGOnormMat)],each=nrow(y)),
     y=y[,1:ncol(y)],labels = sigLabel,adj = 0.5,srt=90)
