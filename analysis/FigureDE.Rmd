...
spcID2spcName <- c( BrDi="Brachypodium", HoVu="Hordeum", MeNu1="Melica", StLa="Stipa", NaSt="Nardus")

# define species colors to use in plots
spcColors <- c( Nardus = "#0049dbff",
                Stipa = "#ff0000ff",
                Melica = "#cd28e3ff",
                Brachypodium = "#00c500ff",
                Hordeum = "#ff7e00ff")
```
...
pooidVennPlot <- function(grpIdsList){
  ...
  do.call(draw.quintuple.venn, args=c( as.list(vennArgs) ,list(
    scaled = F,category=spcNames,fill=spcColorsWht[spcNames],margin = 0.05,
...
# plot.phylo(tree2, show.tip.label = F, type= "u")
# tiplabels( pch=sub(".*T(.).*","\\1",tree2$tip.label,perl=T), col=spcColors[match(substr(tree2$tip.label,1,1),substr(names(spcColors),1,1))])
...
for( timepoint in names(trees)){
  ...
  plot.phylo(tree,y.lim=c(0,100), 
             tip.color=spcColors[match(substr(tree$tip.label,1,1),substr(names(spcColors),1,1))],
             direction = "downwards",main=timepoint,cex = 1)
